...
ImageEraseFileItemDelegate::ImageEraseFileItemDelegate(QObject *parent) :
    QStyledItemDelegate(parent) {
}

ImageEraseFileItemDelegate::~ImageEraseFileItemDelegate() {
}

bool ImageEraseFileItemDelegate::eventFilter(QObject *object, QEvent *event) {
    int type = event->type();
...
void ImageEraseFileItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
...
QSize ImageEraseFileItemDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
    return size_;
}

void ImageEraseFileItemDelegate::changeSizeHint(const QSize &size) {
    size_ = size;
}
...
void ImageEraseFileListView::changeData(QList<Data> datas) {
    file_list_view_->chageData(datas);
}

void ImageEraseFileListView::setCurrentIndex(int index) {
    file_list_view_->setCurrentIndex(file_list_view_->model()->index(index, 0));
}

int ImageEraseFileListView::currentIndex() {
...
void ImageEraseFileListView::createUi() {
    ...
    auto delegate = new ImageEraseFileItemDelegate(this);

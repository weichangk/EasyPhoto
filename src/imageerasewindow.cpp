...
void ImageEraseWindow::changeFileListData(QList<Data> datas) {
    if (datas.count() == 0) {
...
void ImageEraseWindow::setFileListCurrentIndex(int index) {
    file_list_view_->setCurrentIndex(index);
}

int ImageEraseWindow::fileListCurrentIndex() {
    return file_list_view_->currentIndex();
}

void ImageEraseWindow::createUi() {
    ...
    preview_view_ = new ImageErasePreviewView(this);
    viewsWidgetLayout->addWidget(preview_view_, 1);

    setting_view_ = new ImageEraseSettingView(this);

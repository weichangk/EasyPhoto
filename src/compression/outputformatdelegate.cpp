...
void CompressionOutputFormatDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    auto data = index.data(Qt::UserRole).value<SCompressionOuputFormat>();
...
bool CompressionOutputFormatDelegate::eventFilter(QObject *object, QEvent *event) {
    int type = event->type();
...
QSize CompressionOutputFormatDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
    return m_Size;
}
#include "enhancement/inputlistdelegate.h"
...
EnhanceInputListDelegate::EnhanceInputListDelegate(QObject *parent) :
    QStyledItemDelegate(parent) {
}

void EnhanceInputListDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    auto data = index.data(Qt::UserRole).value<SEnhanceInputData>();
...
bool EnhanceInputListDelegate::eventFilter(QObject *object, QEvent *event) {
    int type = event->type();
...
QSize EnhanceInputListDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
    return m_Size;
}